...
@app.get("/")
async def home():
    return {"message": "YouTube Downloader API is Running", "usage": "/download?url=YOUR_URL"}

# --- প্রক্সি এন্ডপয়েন্ট (এটি ফাইলের নাম ঠিক করে দিবে) ---
@app.get("/proxy")
async def proxy_engine(url: str, filename: str):
    ...
    # অডিওর জন্য mpeg আর ভিডিওর জন্য mp4
        ...
        for q in qualities:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if media_url:
                        # ১. ফাইলের নাম তৈরি করা (টাইটেল থেকে সুন্দর নাম)
                        safe_title = re.sub(r'[^\w\s-]', '', video_info['title']).strip().replace(' ', '_')
                        ext = "mp3" if q == "audio" else "mp4"
                        final_filename = f"{safe_title}_{q}.{ext}"

                        # ২. প্রক্সি লিঙ্ক জেনারেট করা
                        ...
                        # ৩. আপনার কাঙ্ক্ষিত ফরম্যাটে ডেটা সাজানো
                        all_links.append({
                            "quality": q if q != "audio" else "MP3 High Quality",
                            "download_url": proxy_link, # এটিই আপনার কাস্টম নামসহ লিঙ্ক
